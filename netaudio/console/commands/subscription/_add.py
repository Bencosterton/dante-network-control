...
class SubscriptionAddCommand(Command):
    ...
    async def subscription_add(self):
        self.line("Starting device discovery...")
        dante_browser = DanteBrowser(mdns_timeout=10.0)  # Increased timeout to 10 seconds
        dante_devices = await dante_browser.get_devices()
        
        # Debug output
        self.line(f"\nFound {len(dante_devices)} devices:")
        for hostname, device in dante_devices.items():
            self.line(f"  - {device.name} ({device.ipv4})")
            if hasattr(device, 'tx_channels'):
                self.line("    Tx channels:")
                for ch_name, ch in device.tx_channels.items():
                    self.line(f"      - {ch_name}")
            if hasattr(device, 'rx_channels'):
                self.line("    Rx channels:")
                for ch_name, ch in device.rx_channels.items():
                    self.line(f"      - {ch_name}")
        
        self.line("\nGetting device controls...")
